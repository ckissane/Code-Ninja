 ...
 */
public class LightScrollPaneConsole extends JComponent {
	...
	public LightScrollPaneConsole(JComponent component) {
		this.setBorder(BorderFactory.createMatteBorder(
                5, 0, 0, 0, Color.decode("#657b83")));
	...
	private void displayVerticalScrollBarIfNecessary(JViewport viewPort) {
		...
		shouldDisplayVerticalScrollBar = true;
	...
	private static class MyScrollBarUI extends BasicScrollBarUI {
		...
		protected void paintThumb(Graphics g, JComponent c, Rectangle thumbBounds) {
			...
			int arc = THUMB_SIZE;
			int x = thumbBounds.x + THUMB_BORDER_SIZE + (orientation == JScrollBar.HORIZONTAL ? THUMB_SIZE : 0);
			int y = thumbBounds.y + THUMB_BORDER_SIZE + (orientation == JScrollBar.VERTICAL ? THUMB_SIZE : 0);

			int width = orientation == JScrollBar.VERTICAL ? THUMB_SIZE : thumbBounds.width - (THUMB_BORDER_SIZE * 2);
			width = Math.max(width, THUMB_SIZE);

			int height = orientation == JScrollBar.VERTICAL
					? thumbBounds.height - (THUMB_BORDER_SIZE * 2) - (THUMB_SIZE * 2) : THUMB_SIZE;
